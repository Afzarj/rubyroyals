    ...
    for (let i = 0; i < num; i++) {
        container.innerHTML += `
            <div class="repayment-block">
                <label>Amount ${i+1} 
                    <input type="number" name="RepayAmount${i+1}" min="0" step="0.01" required oninput="updateRepaymentTotal()">
                </label>
                <label>Date ${i+1} 
...
function updateRepaymentTotal() {
    const total = Array.from(document.querySelectorAll("[name^='RepayAmount']"))
        .reduce((sum, input) => sum + (parseFloat(input.value) || 0), 0);
    const totalInput = document.getElementById("repaymentTotalAmount");
    if (totalInput) {
